...
public class PinActivity extends AppCompatActivity {
    ...
    private void init(Bundle savedInstanceState) {
        if (savedInstanceState == null) {
            ...
            firstValue = "";
            mode = (Session.getInstance().getPasswordHash().isEmpty()) ? MODE_CREATION : MODE_VERIFICATION;
        }
        else {
    ...
    public void onBackPressed() {
        // Disable going back to the activity
        moveTaskToBack(true);
    }
    ...
    public void checkPin(View view) {
        ...
        switch (mode) {
            case MODE_VERIFICATION:
                String hash = Enigma.hashPassword(numberPin.toString(), Base64.decode(Session.getInstance().getPasswordSalt(), Base64.NO_WRAP));
                if (hash.equals(Session.getInstance().getPasswordHash())) {
                    Session.getInstance().setPasswordValue(numberPin.toString());
            ...
            case MODE_CREATION:
                if (firstValue.isEmpty()) {
                    ...
                    if (firstValue.equals(numberPin.toString())) {
                        ...
                        if (hash != null) {
                            // todo перенести в Session? (setPasswordSalt & setPasswordHash)
                            SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(this).edit();
                            editor.putString(Session.APP_PREF_PASSWORD_SALT, Base64.encodeToString(salt, Base64.NO_WRAP));
                            editor.putString(Session.APP_PREF_PASSWORD_HASH, hash);
                            editor.commit();
                            finish();
                        }
                    } else {
