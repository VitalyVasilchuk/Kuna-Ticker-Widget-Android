                ...
                case "RESULT_OFFERS":
                    ...
                    if (offers != null) {
                        ...
                        for (List<String> ask : asks) {
                            ...
                            vol = new BigDecimal(ask.get(1));
                            amount = price.multiply(vol);

                            HashMap<String, String> hashMap = new HashMap<>();
                            hashMap.put("price", Utils.getFormattedValue(ask.get(0)));
                            hashMap.put("vol", Utils.getFormattedValue(ask.get(1)));
                            hashMap.put("amount", Utils.getFormattedValue(amount.toPlainString()));

                            listData.add(0, hashMap); // значения необходимо вставлять в обратном порядке
                        }
                        ((BaseAdapter) listView.getAdapter()).notifyDataSetChanged();
                    }
                    swipeRefreshLayout.setRefreshing(false);
