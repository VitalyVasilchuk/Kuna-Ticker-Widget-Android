 ...
 */
public class UnlimitedDelegate extends UiManagingDelegate {
    public static final String SKU_ID = BillingConstants.SKU_PRIVATE_API_UNLIMITED;

    public UnlimitedDelegate(BillingProvider billingProvider) {
    ...
    public void onBindViewHolder(SkuRowData data, RowViewHolder holder) {
        ...
        int textId = mBillingProvider.isUnlimitedPurchased() ? R.string.button_own
                : R.string.button_buy;
        holder.button.setText(textId);
        holder.skuIcon.setImageResource(R.drawable.private_api_unlimited);
    }

    @Override
    public void onButtonClicked(SkuRowData data) {
        if (data != null && mBillingProvider.isUnlimitedPurchased()) {
